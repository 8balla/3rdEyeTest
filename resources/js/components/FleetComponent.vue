<template>
    <div>
        <div class="justify-content-center d-flex row marginTop above">
            <div class="col-6">
                <a
                    v-show="this.showFleet"
                    class="right btn btn-primary p-4"
                    @mousedown="manage()"
                >
                    Manage Fleet
                </a>
            ...
            <div class="col-6">
                <a
                    v-show="this.addFleet"
                    class="left btn btn-primary p-4"
                    @mousedown="fleets()"
                    >Add Starship</a
                >
            </div>
        ...
        <div class="row justify-content-center">
            <div class="mt-5">
                <div>
                    ...
                    <div
                        class="mt-5 stars"
                        v-if="this.startScroller"
                        id="scroller"
                    >
                        <div>
                            <p>
                                I am sorry i could not resist :) <br /><br />
                            ...
                            <p>
                                ...
                                custodian of the stolen plan that can save her
...
<script>
export default {
    props: ["fleet", "fleetUrl", "createUrl"],
    data() {
        return {
            search: "",
            showFleet: false,
    ...
    created() {
        setTimeout(() => {
            return (
                (this.showFleet = true),
                (this.addFleet = true),
                (this.startScroller = true)
            );
        }, 15000);
        return new axios.get('api/starfleet/').then((response) => {
            this.resp = response.data;
           
           console.log(this.resp, 'resppp')
        });
    },
    computed: {
        showFleets() {
            return this.limit
                ? this.fleet.slice(0, 12, this.limit)
                : this.fleet;
        },
        filteredFleet() {
            return this.props.filter((fleet) => {
                return fleet.name.match(this.search);
            });
        },
    },
    methods: {
        fleets() {
            return window.location.replace("fleet/create");
        },
        manage() {
            return window.location.replace("fleet");
        },
    },
    mounted() {
...
#intro {
    ...
    text-align: center;
    color: rgb(230, 238, 75);
    font-size: 200%;
    animation: intro 100s ease-out normal;
}
...
#logo {
    ...
    animation: logo 100s ease-out normal;
    z-index: 1;
}
...
#scroller div {
    position: fixed;
    top: 10%;
    animation: scroller 100s linear normal;
    z-index: -1;
}
